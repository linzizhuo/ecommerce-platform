...
async function submitOrder() {
  ...
  try {
    const res: any = await request.post('/order', {
    ...
    // 模拟支付
    await request.post(`/order/${orderId}/pay`, { payMethod: payMethod.value })
...
onMounted(async () => {
  ...
  if (cached) items.value = JSON.parse(cached)
  else router.push('/cart')

  try {
    const res: any = await request.get('/address')
...
.inner { max-width: 800px; margin: 0 auto; padding: 20px; }
