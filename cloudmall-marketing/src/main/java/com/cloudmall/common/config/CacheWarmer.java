...
import com.cloudmall.mapper.SeckillSessionMapper;
...
public class CacheWarmer implements CommandLineRunner {
    ...
    /** 预热未结束的秒杀场次 — 缓存场次元数据 + 初始化库存计数器 */
    private void warmSeckillSessions() {
        ...
        for (SeckillSession s : sessions) {
            try {
                seckillService.warmUp(s.getId());
            } catch (Exception e) {
                log.warn("秒杀场次预热失败: sessionId={}, {}", s.getId(), e.getMessage());
            }
        }
        log.info("预热秒杀场次: {} 个", sessions.size());
    }
}
